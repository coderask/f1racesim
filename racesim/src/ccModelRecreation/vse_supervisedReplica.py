...
with open('racesim/src/ccModelRecreation/dumped_data.pkl', 'rb') as f:
    nonNumpyValidationData = pickle.load(f)
# setCount = 0
# checkSame = np.all(nonNumpyValidationData[0] == nonNumpyValidationData[1])
# print("checkSame",checkSame)

for race in training_indicies[0]:
    # print("SET NUMBER", setCount)
    # setCount+=1
    print(race)
    data = nonNumpyValidationData[race]
    training_inputs.append(data[:,:34])
    training_labels.append(data[:,34:]) 
for race in validation_indicies[0]:
    print(race)
    data = nonNumpyValidationData[race]
    validation_inputs.append(data[:,:34])
    validation_labels.append(data[:,34:])


        # print(nonNumpyValidationData[race].shape)
        # training_data.append(nonNumpyValidationData[race])
training_inputs = np.vstack(training_inputs)
training_labels = np.vstack(training_labels)
validation_inputs = np.vstack(validation_inputs)
